...
-define(UPDATE_ERROR, {error, unknown}).
...
greater_insert_fail(Config) ->
  BoundA = ?value(bound_greater_a, Config),
  BoundB = ?value(bound_greater_b, Config),
  {ok, [{error, Msg1}], _} = tutils:aql(?format(insert_greater, [10, BoundA, BoundB+1], Config)),
  {ok, [{error, Msg2}], _} = tutils:aql(?format(insert_greater, [11, BoundA+1, BoundB], Config)),
  {ok, [{error, Msg3}], _} = tutils:aql(?format(insert_greater, [12, BoundA, BoundB-1], Config)),
...
smaller_insert_fail(Config) ->
  BoundA = ?value(bound_smaller_a, Config),
  BoundB = ?value(bound_smaller_b, Config),
  {ok, [{error, Msg1}], _} = tutils:aql(?format(insert_smaller, [10, BoundA, BoundB-1], Config)),
  {ok, [{error, Msg2}], _} = tutils:aql(?format(insert_smaller, [11, BoundA-1, BoundB], Config)),
  {ok, [{error, Msg3}], _} = tutils:aql(?format(insert_smaller, [12, BoundA, BoundB], Config)),
